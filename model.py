...
class Model(tf.keras.Model):
    def __init__(self,image_dim=256,output_scale=3.0):
        ...
        num_filters = 32
        self.conv1_1 = layers.Conv2D(num_filters, (3, 3), padding='same', activation='relu', input_shape=(image_width, image_height, 3))
        self.conv1_2 = layers.Conv2D(num_filters, (3, 3), padding='same', activation='relu', input_shape=(image_width, image_height, 3))
        self.pool1 = layers.MaxPooling2D((2,2))
        self.bn1 = layers.BatchNormalization()

        # Block 2: (Conv(3×3) → Conv(3×3) → MaxPool) 64 filters
        num_filters = 64
        self.conv2_1 = layers.Conv2D(num_filters, (3, 3), padding='same', activation='relu', input_shape=(image_width, image_height, 3))
        self.conv2_2 = layers.Conv2D(num_filters, (3, 3), padding='same', activation='relu', input_shape=(image_width, image_height, 3))
        self.pool2 = layers.MaxPooling2D((2,2))
        self.bn2 = layers.BatchNormalization()

        # Block 3: (Conv(3×3) → Conv(3×3) → MaxPool) 128 filters
        num_filters = 128
        self.conv3_1 = layers.Conv2D(num_filters, (3, 3), padding='same', activation='relu', input_shape=(image_width, image_height, 3))
        self.conv3_2 = layers.Conv2D(num_filters, (3, 3), padding='same', activation='relu', input_shape=(image_width, image_height, 3))
...
def train(args, model, ds):
    ...
    tf_ds = training_ds.to_tf_dataset(columns='x', label_cols='y', batch_size=32, shuffle=True)
